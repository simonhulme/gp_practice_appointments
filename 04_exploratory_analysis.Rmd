...
total_daily_appointments %>% 
    ...
    theme_bw() +
    labs(title = "Bimodal distribution due to zero values for holidays")
...
gp_daily_appointments %>% 
    filter(total_appointments > 0) %>% 
    ggplot(aes(total_appointments)) +
    geom_histogram(bins = 15, fill = "skyblue", color = "grey30") +
    theme_bw() +
    labs(title = "Approximately normal distribution when zero values omitted")
...
levene_test_result <- 
    car::leveneTest(appointments ~ booking_type, 
                    data = booking_type_attended_tbl %>% filter(appointments > 0))
levene_test_result
```
