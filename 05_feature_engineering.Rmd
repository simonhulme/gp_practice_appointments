...
trend_season_event_mdl <-
    lm(
        appointments ~ splines::ns(index.num, df = 4) + wday.lbl + month.lbl +
...
trend_season_events_ar_tbl <- 
    trend_season_events_tbl %>% 
    tk_augment_lags(.value = appointments, .lags = 1:5)
```
...
trend_season_events_ar_tbl %>%
    plot_time_series_regression(
        ...
        .formula = appointments ~ splines::ns(index.num, df = 4) + 
            wday.lbl + month.lbl +
            bank_holiday + training + 
            appointments_lag1 + appointments_lag2 + appointments_lag3 + appointments_lag4 + appointments_lag5,
        .show_summary = TRUE
    )
```
...
trend_season_events_lagged_tbl <-
    trend_season_events_tbl %>% 
    tk_augment_lags(.value = bank_holiday, .lags = 1:3) %>% 
    tk_augment_lags(.value = training, .lags = 1:3)
...
```{r}
trend_season_events_lagged_mdl <-
    lm(appointments ~ splines::ns(index.num, df = 4) + 
            wday.lbl + month.lbl +
            bank_holiday + training + 
            bank_holiday_lag1 + bank_holiday_lag2 + bank_holiday_lag3 +
            training_lag1 + training_lag2 + training_lag3,
...
summary(trend_season_events_lagged_mdl)

## Explore results
trend_season_events_lagged_results <- 
    trend_season_events_lagged_tbl %>% 
    drop_na() %>% 
    select(appointment_date, .observed = appointments, everything()) %>% 
    broom::augment(trend_season_events_lagged_mdl, data = .) %>% 
    mutate(.resid = .observed - .fitted)
...
anova(trend_season_events_lagged_mdl)
```
...
time_feats_events_lags_fourier_tbl <- 
    trend_season_events_lagged_tbl %>% 
    tk_augment_fourier(.date_var = appointment_date, .periods = c(20, 130, 260))
...
time_feats_events_lags_fourier_tbl %>% 
    plot_time_series_regression(
        ...
        .formula = appointments ~ splines::ns(index.num, df = 4) + 
            ...
            bank_holiday_lag1 + bank_holiday_lag2 + bank_holiday_lag3 +
            training_lag1 + training_lag2 + training_lag3 +
            appointment_date_sin20_K1 + appointment_date_cos20_K1 +
            appointment_date_sin130_K1 + appointment_date_cos130_K1 +
            appointment_date_sin260_K1 + appointment_date_cos260_K1,
        .show_summary = TRUE)
```

```{r}
trend_season_events_lagged_fourier_mdl <-
    lm(appointments ~ splines::ns(index.num, df = 4) + 
            wday.lbl + month.lbl +
            bank_holiday + training + 
            bank_holiday_lag1 + bank_holiday_lag2 + bank_holiday_lag3 +
...
summary(trend_season_events_lagged_fourier_mdl)
...
trend_season_events_lagged_results <- 
    ...
    drop_na() %>% 
    select(appointment_date, .observed = appointments, everything()) %>% 
    broom::augment(trend_season_events_lagged_fourier_mdl, data = .) %>% 
    mutate(.resid = .observed - .fitted)
...
anova(trend_season_events_lagged_fourier_mdl)
```

```{r}
## Save best model
write_rds(trend_season_events_lagged_fourier_mdl, "00_models/best_lm.rds")
```
